...
def get_myshortlisting(data):
    try:
        profileCode = data['profile_code']        
        connection = get_connection()
        cursor = connection.cursor()        
        sql_select_query = """select * from profile_master where profile_code in (
                                select tgt_profile_code from kkkr.profile_handshake where src_profile_code = %s)"""           
        cursor.execute(sql_select_query, (profileCode,))
        records = cursor.fetchall()
        resultArray = [];
        for row in records:                     
            x = {
